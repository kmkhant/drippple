...
export class MailService {
  ...
  async sendForgotPasswordEmail(user: User, resetToken: string): Promise<void> {
    ...
    const sendMailDto: SendMailDto = {
      ...
      subject: 'Reset your Drippple Password',
      message: `<p>Hey ${user.name}!</p><p>You can reset your password by visiting this link: <a href="${url}">${url}</a>.</p>. It will expire in 30 minutes.`,
    };
